...
function TrustStatsSection() {
  ...
  const hardwareHighlights = [
    {
      title: 'Laptop & Chip Rework Lab',
      tag: 'Diagnostics',
      desc: 'BGA Reballing & Motherboard Short Tracing',
      image: '/images/laptop_repair.png',
      color: '#0EA5E9',
    },
    {
      title: '4K CCTV & IP NVR Setup',
      tag: 'Surveillance',
      desc: 'Night Vision & Mobile Phone Live Feed',
    ...
    },
    {
      ...
      tag: 'Infrastructure',
      desc: 'Cloud Migration & AMC Maintenance SLA',
      image: '/images/server_room_bg.png',
      color: '#06B6D4',
    },
    {
      title: 'Wi-Fi 6 & Mesh Network',
      tag: 'Networking',
      desc: 'Gigabit LAN, Managed Switches & Firewalls',
      image: '/work-networking.png',
      color: '#F59E0B',
    },
  ];
  ...
  return (
    <Section className="relative -mt-8 md:-mt-12 z-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Hardware & Engineering Showcase Row */}
        <motion.div variants={staggerContainer} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5 mt-6">
          {hardwareHighlights.map((item, idx) => (
            <motion.div
              key={idx}
              variants={staggerItem}
              whileHover={{ y: -4 }}
              className="relative overflow-hidden rounded-2xl bg-gradient-to-b from-white/[0.07] to-white/[0.02] border border-white/10 hover:border-[#0EA5E9]/50 p-4 transition-all duration-300 shadow-xl group flex items-center gap-3.5"
            >
              <img
                src={item.image}
                alt={item.title}
                className="w-14 h-14 rounded-xl object-cover border border-white/10 shadow-md group-hover:scale-105 transition-transform duration-500 shrink-0"
              />
              <div className="overflow-hidden">
                <span
                  className="text-[9px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded inline-block mb-1"
    ...
    <div className="bg-[#0B1120]">
      <SEO 
        ...
        schema={{
          ...
          "address": {
            "@type": "PostalAddress",
            "streetAddress": "47A 1st Cross, Sainagar 2nd Stage, 2nd Main Rd, Vidyaranyapura Post, Chikkabettahalli",
